...
if ("undefined" == typeof(iuxwa)) {iuxwa = new Object();}

/************************** START CONFIG SECTION **************************/
iuxwa.internalDomains = "intuit.com,localhost";

iuxwa.getSuite = function() {
	if(intuit && intuit.ius && intuit.ius.analyticEnv === 'pre-prod') {
		return "intuitctoiuxqa";
	} else {
		return "intuitctoiux";
	}	
};
iuxwa.reportSuite = iuxwa.getSuite();
iuxwa.maxTime = 15;
iuxwa.maxTime2 = 20;
/************************** END CONFIG SECTION **************************/

var fireIUXTrackEvent = function (obj, actionName, labelName, testId, category) {
	iuxs_account=iuxwa.getSuite();
    iuxs = s_gi(iuxs_account);
    iuxs.eVar6 = actionName; 
    iuxs.prop6 = "D=v6"; 
    iuxs.eVar12 = labelName; 
    iuxs.prop12 = "D=v12"; 
    iuxs.eVar13 = testId; 
    iuxs.prop13 = "D=v13";
    iuxs.eVar14 = category;
    iuxs.prop14 = "D=v14";
	iuxs.linkTrackVars = "prop6,eVar6,prop12,eVar12,prop13,eVar13,prop14,eVar14";
	var lt = obj.href != null ? iuxs.lt(obj.href) : "";
	if (lt == "") { iuxs.tl(obj, 'o', actionName, null, 'navigate'); }
};

function iuxTrack(obj, actionName, labelName, testId, category) {
	try {
		//check whether s_code or client insight library is present
		if(window.s || window.CI || window.s_c_il) {
			iuxwa.maxTime--;
			var omnitureCookie = iuxs.c_r('s_vi');
			
			// check whether the cookie is set
			if(omnitureCookie) {
				fireIUXTrackEvent(obj, actionName, labelName, testId, category);
			} else {
				if(iuxwa.maxTime >= 0) {
					setTimeout(function() {
						iuxTrack(obj, actionName, labelName, testId, category);
					}, 500);
				}
				
				if(iuxwa.maxTime < 0) {
					// just fire the event which will set the omniture cookie s_vi even if it's empty
					fireIUXTrackEvent(obj, actionName, labelName, testId, category);
				}
			}
		} else {
...
var fireIUXTrackPageEvent = function (pagename, offeringID) {
	//TODO: clear custom link variable
	iuxs_account=iuxwa.getSuite();;
    iuxs = s_gi(iuxs_account);
	iuxs.prop6 = iuxs.eVar6 = "";
	iuxs.prop12 = iuxs.eVar12 = "";
	
	iuxs.pageName = pagename;
	iuxs.eVar11 = offeringID;
	iuxs.prop11 = "D=v11";
	iuxs.t();
};

function iuxTrackPage(pagename, offeringID){
	try {
		//check whether s_code or client insight library is present
		if(window.s || window.CI || window.s_c_il) {
			iuxwa.maxTime2--;
			...
			// check whether the cookie is set
			if(omnitureCookie) {
				fireIUXTrackPageEvent(pagename, offeringID);
			} else {
				// if no omniture cookie we will try 10 times for 500ms a try for total of 5 seconds
				if(iuxwa.maxTime2 >= 0) {
					setTimeout(function() {
						iuxTrackPage(pagename, offeringID);
					}, 500);
				}
...
/*
* Pre-configure iuxwa Object
***************************
*/
var iuxs_account=iuxwa.getSuite();
var iuxs=s_gi(iuxs_account);

/* Standard Configs */
iuxs.charSet="UTF-8";
iuxs.currencyCode="USD";
iuxs.trackDownloadLinks=true;
iuxs.trackExternalLinks=true;
iuxs.trackInlineStats=true;
iuxs.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,docx,pptx,xlsx";
iuxs.linkInternalFilters="javascript:," + iuxwa.internalDomains;
iuxs.linkLeaveQueryString=false;
iuxs.linkTrackVars="None";
iuxs.linkTrackEvents="None";
iuxs.server=window.location.hostname.toLowerCase();


/*
 	doPlugins() function automatically called by SiteCatalyst implementation.
*/
iuxs.usePlugins=true;
function s_doPlugins(s) {
	s.plugins="";
}

iuxs.doPlugins=s_doPlugins;

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
iuxs.visitorNamespace="intuitinc"
iuxs.dc=122
iuxs.trackingServer="ci.intuit.com"
iuxs.trackingServerSecure="sci.intuit.com"
