...
/*
 * Intuit Web Analytics SDK Version: 0.2
 * @license Copyright 2012 Intuit Inc. All Rights Reserved
 */

/*jshint browser:true, nonstandard:true, regexp:false, scripturl:true */

var wa = (function (document, wa) {
...
//    wa.internalDomainsQA = "localhost";
    ...
    /*
     * Local Variables/Events Mapping
     ***************************
     */
    wa.customProperties = {
        "app_name": "3",
    ...
    wa.customEvents = {
        "create_app": "event1",
        "submit_for_review": "event2",
        "failed_search": "event3",
        "try_api": "event4",
        "search": "event5"
    };
    /*
     ...
     **********************************************************	
    */
    /* SiteCatalyst Predefined Variables */
    wa.properties = {
        "server": "server",
        "campaign": "campaign",
        "site_section": "channel",
	...
	**********************************************************	
   */
    /* SiteCatalyst Predefined Events */
    wa.events = {
    ...
    * properties: optional field.  Use this parameter to pass in any parameter related to the event.  Value must be in json key-value pari format.  All keys need to be registered first.
    */
    wa.trackEvent = function (event, properties) {
        s = s_gi(s_account);

        s["prop" + wa.properties.event_name] = s["eVar" + wa.properties.event_name] = event; //used for link tracking    
        s.linkTrackVars = "prop" + wa.properties.event_name + ",eVar" + wa.properties.event_name;

        if (wa.events.hasOwnProperty(event)) {
            if ((typeof (event) != "undefined") && (event != null)) {
                var eventVar = wa.events[event];
                s.events = eventVar;
                s.linkTrackVars += ",events";
                s.linkTrackEvents = eventVar;
            }
        }
        if ((typeof (properties) != "undefined") && (properties != null)) {
            for (var key in properties) {
                if (wa.properties.hasOwnProperty(key)) {
                    var scKey = wa.properties[key];
                    if (scKey == wa.properties.event_id && s.events != null) //event dedupID
                    {
                        s.events += ":" + properties[key];
                    }
                    else if (scKey == wa.properties.hierarchy) //hierarchy
                    {
                        var hierarchies = properties[key];
                        for (var i = 0; i < hierarchies.length; i++) {
                            s["hier" + (i + 1)] = '';
                            for (var j = 0; j < hierarchies[i].length; j++) {
                                if (s["hier" + (i + 1)].length > 0) s["hier" + (i + 1)] += "|";
                                s["hier" + (i + 1)] += hierarchies[i][j];
                            }
                            s.linkTrackVars += "," + "hier" + (i + 1);
                        }
                    }
                    else if (scKey == wa.properties.products) //products
                    {
                        s[wa.properties.products] = getProductStr(properties[key]);
                        s.linkTrackVars += "," + wa.properties.products;
                    }
                    else {
                        if (isNaN(scKey)) //SC Predefinied Variables
                        {
                            s[scKey] = properties[key];
                            s.linkTrackVars += "," + scKey;
                        }
                        else //Custom Variables
                        {
                            s["prop" + scKey] = properties[key];
                            s["eVar" + scKey] = properties[key];

                            s.linkTrackVars += ",prop" + scKey + ",eVar" + scKey;
                        }
                    }
                }
                else //Context Data Variables
                {
                    s.contextData[key] = properties[key];
    ...
    wa.registerOnErrorHandler = function () {
        
        var originalImage = window.Image;
        window.Image = function() {
            var im = new originalImage();
            try {
                im.onerror = function () {
                    ...
                    var wd = window, s;
                    if (wd.s_c_il) {
                        for (var i = 0; i < wd.s_c_il.length; i++) {
                            s = wd.s_c_il[i];
                            if (s.nrs) {
                                s.nrs--;
                            }
                ...
                console.log("exception in on error handler called");
            }
            return im;
    ...
    * Page tracking
    * Input Parameters:
    * pageName: optional field. page name up to 100 characters
    * event: optional field.  Use this parameter to pass in any event that needs to be incremented. Example: wa.events.Login
    * properties: optional field.  Use this parameter to pass in any parameter related to the event.  Value must be in json key-value pari format.  All keys need to be registered first.
    */
    wa.trackPage = function (pageName, event, properties) {

        clearVariables();

        if (!pageName) {
            var curl = location.href.toLowerCase();
            var urlRegExp = /(\w+):\/\/([\w.:-]+)\/?([\w.\/-~]*\/)?([^\/?]*)\??(\S*)/; //split url into protocol, domain, path, and file
            var urlArray = curl.match(urlRegExp);
            var defaultPath = (urlArray && urlArray[3]) || "";
            var defaultPage = (urlArray && urlArray[4]) || "index";

            if (defaultPage) { defaultPage = defaultPage.substr(0, defaultPage.lastIndexOf('.')) || defaultPage; } //strip out file extension
            if (defaultPath) { defaultPath = defaultPath.replace(/\/$/g, ''); } // remove trailing forward slash (/)

            if (defaultPath) {
                pageName = (defaultPage) ? defaultPath + "/" + defaultPage : defaultPath;
            } else {
                pageName = (defaultPage) ? defaultPage : "";
            }
        }
        if (pageName.charAt(0) == '/') {
            pageName = pageName.substring(1); // leave the first char and get the rest
        }
        s.pageName = wa.siteName + "|" + pageName.toLowerCase();

        if (wa.events.hasOwnProperty(event)) {
            s.events = wa.events[event];
        }
    ...
    function clearVariables() {
        for (var key in wa.properties) {
            var scKey = wa.properties[key];

            if (isNaN(scKey)) {     // SC Predefinied Variables
                s[scKey] = null;
            } else {                // Custom Variables
                s["prop" + scKey] = s["eVar" + scKey] = null;
            }

            s["events"] = null;
            s["hier1"] = s["hier2"] = s["hier3"] = s["hier4"] = s["hier5"] = null;
        }
    }

    function isHostInDomainList(hostName, domainList) {
        var pattern, x,
            domainPatternArray = (domainList ? domainList.replace(/\./g, "\\\.").split(",") : []); // split the domains into an array and escape the periods for pattern matching
        for (var x = 0; x < domainList.length; x++) {
            pattern = "\\b" + domainPatternArray[x] + "$"; //match either the entire element as the domain,or if there is a sub-domain, match the domain at the end
            if (hostName.search(pattern) >= 0) {
                return true; //return true if the domain pattern matches
            }
        }
        return false; //if no matches, return false
    }

    function getProductStr(prodAry) {
        var retValue = '';
        if (prodAry != null) {
            for (var i = 0; i < prodAry.length; i++) {
                var category = '';
                var name = '';
                var quantity = '';
                var price = '';
                if (prodAry[i]["category"] != null)
                    category = prodAry[i]["category"];
                if (prodAry[i]["name"] != null)
                    name = prodAry[i]["name"];
                if (prodAry[i]["quantity"] != null && !isNaN(prodAry[i]["quantity"]))
                    quantity = prodAry[i]["quantity"];
                if (prodAry[i]["price"] != null && !isNaN(prodAry[i]["price"]))
                    price = prodAry[i]["price"];
                if (name.length > 0) {
                    if (retValue.length > 0) retValue += ",";
                    retValue += category;
                    retValue += ';' + name;
                    if (quantity.length > 0 || price.length > 0)
                        retValue += ';' + quantity + ';' + price;
                }
    ...
    /*
     * Private SiteCatalyst Functions
     **********************************
     */
    // s_doPlugins() is called whenever s.t() or s.tl() are called or when exit/download link is clicked
    // Put here all default values to variables that apply to every tracking
    function s_doPlugins(s) {
        /*
       * Cature Global Standard Variable
       *********************************
       */
        s[wa.properties.server] = s[wa.properties.server] = window.location.hostname.toLowerCase(),
        wa.siteName = (wa.siteName && wa.siteName.toLowerCase()) || "";
        s["prop" + wa.properties.site_name] = s["eVar" + wa.properties.site_name] = wa.siteName;
        s["eVar" + wa.properties.current_url] = curl;
        s["prop" + wa.properties.current_url] = curl.replace(/^\/(.*)/, "$1");
        var d = new Date();
        s["prop" + wa.properties.current_date] = s["eVar" + wa.properties.current_date] = d.getUTCFullYear() + "|" + (d.getUTCMonth() + 1) + "|" + d.getUTCDate();

        // capture referring url
        if (!isHostInDomainList(rurl.replace(/(\/\/[^\/]+\/).*/, '$1').replace(/www\.|http:|https:|\//g, ""), wa.internalDomains)) {
            s["eVar" + wa.properties.visit_referral_url] = rurl;
        }

        // The cross-domain campaign tracking variable that value is pulled from a query string parameter.
        var xcid = s.getQueryParam('xcid', '', curl);
        if (xcid) {
            s["prop" + wa.properties.xcid] = s["eVar" + wa.properties.xcid] = xcid;
        }

        // The within-domain campaign tracking variable that is pulled from a query string parameter.
        var scid = s.getQueryParam('scid', '', curl);
        if (scid) {
            s["prop" + wa.properties.scid] = s["prop" + wa.properties.scid] = scid;
        }

        // Campaign
        var cid = s.getQueryParam('cid', '', curl);
        if (cid) {
            s["prop" + wa.properties.cid] = s[wa.properties.campaign] = cid;
        }

        //Test & Target Integration
        s["prop" + wa.properties.tnt] = s["eVar" + wa.properties.tnt] = s.trackTNT();

        //Page Loading Time
        if (wa.startTime) {
            var endTime = new Date();
            var diff = new Date();
            diff.setTime(endTime - wa.startTime);
            s["prop" + wa.properties.loading_time] = s["eVar" + wa.properties.loading_time] = diff.getMilliseconds();
        }
     ...
     *********************
     */
    var s = {}, //SiteCatalyst s object
        s_account = "", // report suite name - dynamically selected based on current host
        s_defaults = {},  // default configuration - can be overridden by wa properties
        config = "",
        curl = location.href.toLowerCase(),
        rurl = document.referrer.toLowerCase();

    for (var key in wa.customProperties) {
        wa.properties[key] = wa.customProperties[key];
    }
    for (var key in wa.customEvents) {
        wa.events[key] = wa.customEvents[key];
    }
    ...
    s_defaults = {
        ...
        linkDownloadFileTypes: "exe,zip,gz,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,docx,pptx,xlsx",
        linkInternalFilters: "javascript:," + wa.internalDomains,
    ...
    // popuplate s object with standard configs unless specified by user via wa object
    for (config in s_defaults) {
        if (s_defaults.hasOwnProperty(config)) {
            s[config] = (typeof wa[config] !== "undefined") ? wa[config] : s_defaults[config];
        }
    }

    // set to call s_doPlugins() whenever s.t() or s.tl() is called
    s.usePlugins = true;
    s.doPlugins = s_doPlugins;
    // add various plugin functions to s object
    s_addPlugins(s);



    /*
    * SiteCatalyst Plug-in Functions
    * s_addPlugins() is called after creating the s object
    * Includes all needed plugins provided by SiteCatalyst
    **********************************************************************
    */
    function s_addPlugins(s) {
    ...
    /*
    * Omniture Account ConfigurationConfigure s Object
    ...
    */
    ...
    // SiteCatalyst constructor function for s object
